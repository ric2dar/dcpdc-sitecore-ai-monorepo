# DCPDC Sitecore AI - Demo Site (NextJS) - nextjsstarter
...
DCPDC Sitecore AI is a simple website with sample Component implementation showcasing data source handling and other essentials. This demo site is built to showcase SitecoreAI CMS capabilities using the Content SDK.
...
1. Clone the repository (if not yet done)
    ```git clone https://github.com/kcunanan/dcpdc-sitecore-ai-rendering.git```
2. Starting from the root of the repository navigate to site app folder
    ```cd dcpdc-sitecore-ai-rendering\```
...
6. Install dependencies:
   from ```dcpdc-sitecore-ai-rendering``` run ```npm install```
